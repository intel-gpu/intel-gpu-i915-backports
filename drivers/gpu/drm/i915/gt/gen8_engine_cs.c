...
static u32 hwsp_offset(const struct i915_request *rq)
{
	...
	tl = rcu_dereference_protected(rq->timeline, true);
...
int gen8_emit_init_breadcrumb(struct i915_request *rq)
{
	...
	if (intel_timeline_is_relative(tl)) {
		...
		cmd = MI_STORE_DWORD_INDEX;
		if (intel_timeline_in_context(tl))
			cmd |= MI_USE_PPHWSP;

		*cs++ = cmd;
		*cs++ = offset_in_page(offset);
...
int xehp_emit_bb_start(struct i915_request *rq,
		       ...
		       const unsigned int flags)
{
	u32 arb = MI_ARB_ENABLE;

	if (unlikely(i915_request_has_nopreempt(rq)))
		arb = MI_ARB_DISABLE;

	return __xehp_emit_bb_start(rq, offset, len, flags, arb);
}
...
static u32 *emit_breadcrumb(struct i915_request *rq, u32 *cs)
{
	...
	/* Signal the user fence first (after flushing memory/GO) */
	cs = emit_user_fence_store(rq, cs);

	/* so that the user fence itself can be guarded by the completion. */
	if (intel_timeline_is_relative(tl)) {
		u32 cmd;
...
u32 *gen12_emit_fini_breadcrumb_rcs(struct i915_request *rq, u32 *cs)
{
	...
	cs = gen12_emit_pipe_control(cs, PIPE_CONTROL0_HDC_PIPELINE_FLUSH, flags, LRC_PPHWSP_SCRATCH_ADDR);
