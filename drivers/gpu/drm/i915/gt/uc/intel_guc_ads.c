				...
				struct intel_engine_cs *engine)
{
	...
	if (HAS_EU_STALL_SAMPLING(i915)) {
		ret |= GUC_MCR_REG_ADD(gt, regset, XEHPC_EUSTALL_BASE, false);
		ret |= GUC_MCR_REG_ADD(gt, regset, XEHPC_EUSTALL_BASE_UPPER, false);
		ret |= GUC_MCR_REG_ADD(gt, regset, XEHPC_EUSTALL_CTRL, true);
		ret |= GUC_MCR_REG_ADD(gt, regset, GEN8_ROW_CHICKEN2, true);
	}
...
static void guc_waklv_init(struct intel_guc *guc)
{
	...
	if ((GUC_FIRMWARE_VER(guc) >= MAKE_GUC_VER(70, 21, 1)) &&
	    (IS_DG2(gt->i915) || IS_PONTEVECCHIO(gt->i915)))
