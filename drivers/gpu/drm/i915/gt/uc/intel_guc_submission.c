...
static void guc_engine_reset_prepare(struct intel_engine_cs *engine)
{
	...
	/*
	 * Wa_22011802037:
	 * FIXME: stopping the cs times out occassionally. Need to check with HW
	 * is the timeout configured for stop_cs is good enought to rule out any
	 * active MI FORCE WAKEUPs. Until then discard the return value.
	 */
...
static void guc_context_exit(struct intel_context *ce)
{
...
static inline void guc_init_lrc_mapping(struct intel_guc *guc)
{
	...
	for_each_engine(engine, gt, id) {
		...
		if (engine->blitter_context)
			guc_kernel_context_pin(guc, engine->blitter_context);
		if (engine->evict_context)
			guc_kernel_context_pin(guc, engine->evict_context);
		if (engine->bind_context)
			guc_kernel_context_pin(guc, engine->bind_context);
	}
}
...
static void guc_default_vfuncs(struct intel_engine_cs *engine)
{
	...
	/*
	 * FIXME: guc_engine_reset_prepare causes media workload hang for PVC.
	 * Until root caused, disable this for PVC.
	 */
	if (IS_SRIOV_VF(engine->i915) || IS_PONTEVECCHIO(engine->i915))
...
static void guc_context_replay(struct intel_context *ce)
{
	...
	__guc_reset_context(ce, ALL_ENGINES);
	tasklet_hi_schedule(&sched_engine->tasklet);
}
...
struct intel_context *intel_guc_active_context_get(struct intel_engine_cs *engine)
{
	struct intel_guc *guc = &engine->gt->uc.guc;
